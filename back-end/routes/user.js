...
router.delete("/delUser", async (req, res) => {
  ...
  try {
    const deletedUser = await UserModel.findOneAndDelete({ _id: idFromToken });
    if (!deletedUser) {
      return res.status(404).json({ messaage: "user not found" });
    } else {
      return res.status(200).json({ messaage: "user deleted succesfully" });
    }
  } catch (error) {
    return res.status(400).json({ messaage: "error" });
  }
});

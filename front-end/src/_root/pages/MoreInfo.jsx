...
const MoreInfo = () => {
  // Utiliser le hook useState pour stocker l'état de clickedPost
  const [clickedPost, setClickedPost] = useState(null);

  useEffect(() => {
    // Récupérer la chaîne JSON du sessionStorage
    const storedPost = sessionStorage.getItem("clickedPost");
    // Si la chaîne JSON est présente
    if (storedPost) {
      // Convertir la chaîne JSON en objet JavaScript
      const parsedPost = JSON.parse(storedPost);
      // Mettre à jour l'état clickedPost avec l'objet JavaScript
      setClickedPost(parsedPost);
    }
  }, []); // L'effet s'exécutera une seule fois après le premier rendu

  // Si clickedPost est null ou undefined, retourner un composant de chargement ou un message
  if (!clickedPost) {
    ...
    <div className="mx-auto flex my-11  w-2/3 transform overflow-hidden rounded-lg bg-white dark:bg-slate-800 shadow-md duration-300  hover:shadow-lg">
      <div className="p-4 ">
        ...
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          ...
          Places disponibles pour {clickedPost.place} personnes
        </p>
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
        ...
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          📅 Date et heure de départ: {clickedPost.date.substring(0, 9)} ,{" "}
          {clickedPost.date.substring(11, 16)}{" "}
        </p>
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          ✅ Places disponibles: {clickedPost.place}
        </p>
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          💬 Description :
        </p>
        ...
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          📞 Contact: {clickedPost.contact}
        </p>
        <p className="my-3 text-base dark:text-gray-300 text-gray-700">
          🚦 Conditions: Veuillez prévoir d&apos;arriver à l&apos;heure convenue
          pour le départ. Le partage des frais se fera équitablement entre les
          passagers.
        </p>
